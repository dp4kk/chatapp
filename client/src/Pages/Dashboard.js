...
const Dashboard = () => {
  ...
  return (
    ...
    <IconButton className={classes.icon} onClick={()=>setDarkMode(!darkMode)}><Brightness4Icon fontSize='large'/></IconButton>
      ...
      {notificationMessage && <Snackbar open={messageOpen} autoHideDuration={3000} onClose={()=>setMessageOpen(false)} anchorOrigin={{vertical:'top',horizontal:'left'}}><Alert severity='success'>{notificationMessage}</Alert></Snackbar>}
      </React.Fragment>
  );
};
